...
public class RealtorService {
    ...
    @Transactional
    public ResponseDto<String> RealtorSignup(MemberRequestDto memberRequestDto) {
        ...
        Realtor realtor  = new Realtor(memberRequestDto, Authority.ROLE_Member);

        memberRepository.save(realtor);
        return ResponseDto.success("회원가입 완료");
    }

    // 로그인
    @Transactional
    public ResponseDto<LoginResponseDto> RealtorLogin(LoginRequestDto loginRequestDto, HttpServletResponse response) {
        // 이메일 있는지 확인
        Realtor realtor = memberRepository.findByEmail(loginRequestDto.getEmail()).orElseThrow(
                () -> new RequestException(ErrorCode.LOGIN_NOT_FOUND_404)
        );
        // 비밀번호 있는지 확인
        if (!passwordEncoder.matches(loginRequestDto.getPassword(), realtor.getPassword())) {
        ...
        return ResponseDto.success(
                LoginResponseDto.builder()
                        .authority(realtor.getAccount_type())
                        .build()
        );
    }
